...
def prepare(train_data,train_params,arch_gcn,use_locality,load_weight,dataset):
    ...
    minibatch = Minibatch(adj_full_norm, adj_train, role, class_arr, placeholders, train_params, use_locality, load_weight, dataset)
    model = GraphSAINT(num_classes, placeholders, feats, arch_gcn, train_params, adj_full_norm, logging=True)
    ...
    misc_stats = tf.summary.merge([_misc_val_f1_micro,_misc_val_f1_macro,_misc_train_f1_micro,_misc_train_f1_macro, _misc_time_per_epoch,_misc_size_subgraph])
...
def train(train_phases,model,minibatch, sess,train_stat,ph_misc_stat,summary_writer):
    import time
    saver=tf.train.Saver()
    save_record = False
    epoch_ph_start = 0
    f1mic_best, e_best = 0, 0
    time_calc_f1, time_train, time_prepare, time_sample, eval_time = 0, 0, 0, 0, 0
    ...
    for ip,phase in enumerate(train_phases):
        ...
        sample_ed = time.time()
        time_sample = sample_ed - sample_st
        print("incert time",sample_ed-incert," s")
        ...
        for e in range(epoch_ph_start,int(phase['end'])):
            ...
            time_train_ep, time_prepare_ep, time_eval_ep = 0, 0, 0
            while not minibatch.end():
                ...
                if args_global.timeline:      # profile the code with Tensorflow Timeline
                    _,__,loss_train,pred_train = sess.run([train_stat[0], model.opt_op, model.loss, model.preds], feed_dict=feed_dict, options=options, run_metadata=run_metadata)
                    fetched_timeline = timeline.Timeline(run_metadata.step_stats)
                    chrome_trace = fetched_timeline.generate_chrome_trace_format()
                    many_runs_timeline.append(chrome_trace)
                else:
                    _,__,loss_train,pred_train = sess.run([train_stat[0], model.opt_op, model.loss, model.preds], feed_dict=feed_dict, options=tf.RunOptions(report_tensor_allocations_upon_oom=True))
                t2 = time.time()
                time_train_ep += t2-t1
            ...
            if args_global.cpu_eval:      # Full batch evaluation using CPU
                ...
                sess_eval=sess
            loss_val,f1mic_val,f1mac_val,time_eval = evaluate_full_batch(sess_eval,model,minibatch,many_runs_timeline,mode='val')
            t6 = time.time()
            time_eval_ep += t6-t5
            eval_time += time_eval_ep
    ...
    Parameter_n, Parameter_s, weight_t, dataset_name = minibatch.configRT()
    timelines = TimeLiner()
    ...
    print("-"*20,"RESULT","-"*20)
    print("Pure sampling: ", time_sample - weight_t)
    print('Total training time: ', time_train + time_sample - weight_t)
    print("Evaluation time (on CPU): ", eval_time)
    
    #When the weight has been pre-computed, 'weight_t' that denotes the time cost of computing weight, is set to zero invariably. 
    #And it ('weight_t') becomes meaningful only when we are interested in offline execution time.
    
    #The evaluation procedure on Amazon dataset may take non-trivial time, since it is executed on the CPU platform. 
    #One can modify the shell script by removing the '--cpu_eval' flag on last.
...
def train_main(argv=None):
    train_params,train_phases,train_data,arch_gcn,use_locality,load_weight,dataset = parse_n_prepare(args_global)
    model,minibatch,sess,train_stat,ph_misc_stat,summary_writer = prepare(train_data,train_params,arch_gcn,use_locality,load_weight,dataset)
