...
class Minibatch:
    ...
    def __init__(self, adj_full_norm, adj_train, role, class_arr, placeholders, train_params, use_locality, load_weight, dataset, **kwargs):
    ...
    def set_sampler(self,train_phases):
        ...
        if self.use_locality:
            print("##############use locality-aware sampling##############")
            self.size_subg_budget = train_phases['size_subgraph']
            self.graph_sampler = LocalityBasedNodeSampling(self.adj_train,self.node_train,self.size_subg_budget, self.load_weight, self.dataset)
        else:
            print("##############use vanilla node sampling##############")
            self.size_subg_budget = train_phases['size_subgraph']
            self.graph_sampler = NodeSamplingVanillaPython(self.adj_train,self.node_train,self.size_subg_budget)

        self.norm_loss_train = np.zeros(self.adj_train.shape[0])
        self.norm_aggr_train = np.zeros(self.adj_train.size).astype(np.float32)
        tot_sampled_nodes = 0
        s = time.time()
        print("Before sampling loop: ",s-set_sampler_start," s")
        print("Start sampling loop ......")
        par_graph_sample = 0
        while True:
            sample_st = time.time()
            self.par_graph_sample('train')
            par_graph_sample = par_graph_sample + (time.time() - sample_st)
            ...
            if tot_sampled_nodes > self.sample_coverage*self.node_train.size:
                break
        e = time.time()
        print("Sampling loop consumes: ", e-s," s")
        print("Par_graph_sample consumes: ", par_graph_sample)
        s1 = time.time()
        ...
        for v in range(self.adj_train.shape[0]):
            ...
            val = np.clip(self.norm_loss_train[v]/self.norm_aggr_train[i_s:i_e], 0, 1e6)
        ...
        print("After sampling in minibatch",time.time()-s1," s")    

    def configRT(self):
        return self.graph_sampler.configReturn()
    ...
    def feed_dict(self,mode='train'):
        ...
        if mode in ['val','test']:
            ...
            norm_aggr(adj.data.astype(np.float32),adj_edge_index.astype(np.int32),self.norm_aggr_train,num_proc=args_global.num_cpu_core)
            tt2 = time.time()
